...
对于数组 `A`, 其长度为 $n$, 令
$$
S[i] = \sum_{j = 0}^{i} A[j], 0 \leqslant i \leqslant n - 1
$$
...
若令
$$
S[0] = A[0], S[i] = S[i - 1] \; {\rm{xor}} \; A[i], 0 \leqslant i \leqslant n - 1
$$
...
类似地, 有 `S` 满足
$$
S[i][j] = \sum_{p = 0}^{i} \sum_{q = 0}^{j} A[p][q], 0 \leqslant i \leqslant m - 1, 0 \leqslant j \leqslant n - 1
$$
...
常用 `priority_queue<int, vector<int>, greater<int>> pq;` 和 `priority_queue<int, vector<int>, less<int>> pq;` 其中 `greater<int>` 用于小根堆 和 `less<int>` 用于大根堆.