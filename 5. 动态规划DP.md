# 状态与状态转移

### 例1. 数字三角形: 给出一个数字三角形, 从顶点出发每次向下或向右下走一步, 直至最底层, 将途经的数字相加, 得到最大值. 如,
$$
\begin{matrix}
    7 \\
    3 & 8 \\
    8 & 1 & 0 \\
    2 & 7 & 4 & 4 \\
    4 & 5 & 2 & 6 & 5
\end{matrix}
$$
解为 $7 \rightarrow 3 \rightarrow 8 \rightarrow 7 \rightarrow 5 = 30$.

> 定义状态 `dp[i][j]` 为从第 `1` 行第 `1` 列出发, 走到第`i` 行第 `j` 列所有路径的最大和. 有
> 
> `dp[i][j] = max(dp[i - 1][j - 1], dp[i - 1][j]) + matrix[i][j]`,
> 
> 其中, `dp[1][1] = matrix[1][1]` 且 $1 < i \leq n, 1 < j \leq i$. 则最终结果为 `dp[n][1...n]` 的最大值.

## 状态

明确两点:

* 初始状态. 如在例1中, 初始状态为 `dp[1][1] = matrix[1][1]`, `dp` 其余元素应初始化为一个合适的最小数, 如 `INT_MIN`.

* 答案选取位置. 如在例1中, 最终结果为 `max(dp[n][1...n])`.

## 状态转移

* 注意顺序, 要保证转移到当前状态的原状态已经处理完毕.

### 例2. 最长上升子序列( $LIS$ )

给定一个序列, 求其 $LIS$. 子序列定义: 将原序列删除 $0$ ~ $n$ 个元素所组成的序列. 子序列不一定是连续的元素.

> 令 `dp[i]` 表示以原序列下标为 `i` 结尾的上升子序列的最长长度. 取初始状态对所有 `i` 有 `dp[i] = 1`, 答案为 `max(dp[0...n - 1])`.
>
> 对下标为 `i` 的元素, 有 `j` 从 `0` 至 `i - 1` 依次更新
>
> `dp[i] = nums[j] <= nums[i] ? max(dp[i], dp[j] + 1) : dp[i]`.

* [最大连续子序列和](https://leetcode-cn.com/problems/maximum-subarray/)

* [最长公共子序列 ( $LCS$ )](https://leetcode-cn.com/problems/longest-common-subsequence/)

# 背包问题

## 详见 [背包九讲](背包九讲.pdf).

本节代码所对应的题在[AcWing](https://www.acwing.com/problem/)上.

### 1. 01背包问题

朴素算法:
```c++
#include <bits/stdc++.h>
using namespace std;

const int MAXSIZE = 1005;
int N, V, dp[MAXSIZE][MAXSIZE], v[MAXSIZE], w[MAXSIZE];

int main()
{
    cin >> N >> V;
    for (int i = 1; i <= N; i++)
        cin >> v[i] >> w[i];

    for (int i = 1; i <= N; i++)
    {
        for (int j = 1; j <= V; j++)
        {
            if (j < v[i])
                dp[i][j] = dp[i - 1][j];
            else
                dp[i][j] = max(dp[i - 1][j], dp[i - 1][j - v[i]] + w[i]);
        }
    }
    
    int res = 0;
    for (int j = 1; j <= V; j++)
        res = max(res, dp[N][j]);
    cout << res;

    return 0;
}
```

优化空间复杂度:
```c++
#include <bits/stdc++.h>
using namespace std;

const int MAXSIZE = 1005;
int N, V, dp[MAXSIZE], v[MAXSIZE], w[MAXSIZE];

int main()
{
    cin >> N >> V;
    for (int i = 1; i <= N; i++)
        cin >> v[i] >> w[i];

    for (int i = 1; i <= N; i++)
        for (int j = V; j >= v[i]; j--)
            dp[j] = max(dp[j], dp[j - v[i]] + w[i]);

    cout << dp[V];

    return 0;
}
```

### 2. 完全背包问题

```c++
#include <bits/stdc++.h>
using namespace std;

const int MAXSIZE = 1005;
int N, V, v[MAXSIZE], w[MAXSIZE], dp[MAXSIZE];

int main()
{
    cin >> N >> V;
    for (int i = 1; i <= N; i++)
        cin >> v[i] >> w[i];

    for (int i = 1; i <= N; i++)
        for (int j = v[i]; j <= V; j++)
            dp[j] = max(dp[j], dp[j - v[i]] + w[i]);

    cout << dp[V];

    return 0;
}
```

### 3. 多重背包问题

* 朴素算法
  
    ```c++
    #include <bits/stdc++.h>
    using namespace std;

    const int MAXSIZE = 1005;
    int N, V, dp[MAXSIZE];

    int main()
    {
        cin >> N >> V;
        int v, w, s;
        for (int i = 1; i <= N; i++)
        {
            cin >> v >> w >> s;
            for (int j = V; j >= v; j--)
                for (int k = 1; k <= s && k * v <= j; k++)
                    dp[j] = max(dp[j], dp[j - k * v] + k * w);
        }

        cout << dp[V];

        return 0;
    }
    ```

* 二进制优化
    ```c++
    #include <bits/stdc++.h>
    using namespace std;

    const int MAXSIZE = 2005;
    int N, V, dp[MAXSIZE];

    int main()
    {
        cin >> N >> V;
        int v, w, s;
        for (int i = 1; i <= N; i++)
        {
            cin >> v >> w >> s;
            /*
             * 当前物品数量足够将背包装到不能再装此物品, 为完全背包问题 
            **/
            if (s * v >= V)
            {
                for (int j = v; j <= V; j++)
                    dp[j] = max(dp[j], dp[j - v] + w);
            }
            /*
             * 当前物品全部装入后, 背包容量还可继续装当前物品,
             * 为多重背包问题, 二进制优化为多个01背包问题
            **/
            else
            {
                for (int k = 1; k < s; k <<= 1)
                {
                    for (int j = V; j >= k * v; j--)
                        dp[j] = max(dp[j], dp[j - k * v] + k * w);
                    s -= k;
                }
                for (int j = V; j >= s * v; j--)
                    dp[j] = max(dp[j], dp[j - s * v] + s * w);
            }
        }

        cout << dp[V];

        return 0;
    }
    ```

* 单调队列优化*
  
  预备知识: 滑动窗口与单调队列, 例🔗[LeetCode-239. Sliding Window Maximum](https://leetcode-cn.com/problems/sliding-window-maximum/)
  
    ```c++
    #include <bits/stdc++.h>
    using namespace std;

    const int MAXSIZE = 20005;
    int N, V, dp[MAXSIZE], g[MAXSIZE], q[MAXSIZE];

    int main()
    {
        cin >> N >> V;
        int v, w, s;
        for (int i = 1; i <= N; i++)
        {
            cin >> v >> w >> s;
            memcpy(g, dp, sizeof(dp));

            /*
            * 对于任意一种物品而言, 其一定是以 v 个容量放入背包的. 因此,
            * 对于任意状态下的已占用容量, 都可以分成模 v 的余数, 即 v 类情况.
            * 此时对于任意的 j, 只需要向 v 的倍数转移.
            **/
            for (int j = 0; j < v; j++)
            {
                /*
                * front 指向队首, rear 指向队尾, front > rear 时表示队空.
                * q 为单调队列, 用于维护滑动窗口中的最大值的数组下标.
                * v 的个数 = (下标 - 余数) / v = (q[front] - j) / v.
                * 价值 = v * w = (k - j) / v * w.
                **/
                int front = 0, rear = -1;
                for (int k = j; k <= V; k += v)
                {
                    dp[k] = g[k];
                    // 若当前窗口的内容超过了 s 个 (k - q[front] > s * v)
                    if (front <= rear && k - s * v > q[front])
                        front++;
                    
                    /*
                     * dp[k] = max(i - 1 的 dp[k], i - 1 的 dp 中能
                     * 转移的最大 + 可以装入的个数 * 单个价值)
                    **/
                    if (front <= rear)
                        dp[k] = max(dp[k], g[q[front]] + (k - q[front]) / v * w);
                    while (front <= rear && g[q[rear]] - (q[rear] - j) / v * w <= g[k] - (k - j) / v * w)
                        rear--;
                    q[++rear] = k;
                }
            }
        }

        cout << dp[V];

        return 0;
    }
    ```

* 小结*
  
  对于多重背包问题, 其和完全背包问题的不同之处在于, 完全背包问题不限制同一种物品放的个数, 而在多重背包问题中, 每种物品的个数 $s_i$ 可能会限制其转移的状态数, 即当前放入后的容量与此容量的前一个状态之差与当前放入的这种物品的体积(花费)之比不能超过 $s_i$, 否则是无效的转移.

  当 $s_i$ 限制转移的状态时, 在转移的过程中相当于存在一个大小为 $s_i$ 的滑动窗口. 故每次要将之前的值, 与滑动窗口中的最大值比较, 而非与所有转移的最大值比较.


### 4. 混合三种背包问题

将前面的1、2、3中的三种背包问题混合起来, 即有的物品只可以取一次(01背包), 有的物品可以取无限次(完全背包), 有的物品取得次数有一个上限(多重背包). 解决这样的问题, 只需要对每一种物品所属的问题进行判断. 伪代码见下:

```c++
for (int i = 0; i <= N; i++)
{
    if (第i件物品属于01背包)
        ZeroOnePack(F, c[i], w[i]);
    else if (第i件物品属于完全背包)
        CompletePack(F, c[i], w[i]);
    else if (第i件物品属于多重背包)
        MultiplePack(F, c[i], w[i], s[i]);
}
```

[题](https://www.acwing.com/problem/content/7/)解如下:

```c++
#include <bits/stdc++.h>
using namespace std;

const int MAXSIZE = 1005;
int N, V, dp[MAXSIZE];

void ZeroOnePack(int v, int w)
{
    for (int j = V; j >= v; j--)
        dp[j] = max(dp[j], dp[j - v] + w);
}

void CompletePack(int v, int w)
{
    for (int j = v; j <= V; j++)
        dp[j] = max(dp[j], dp[j - v] + w);
}

void MultiplePack(int v, int w, int s)
{
    if (s * v >= V)
        CompletePack(v, w);
    else
    {
        for (int k = 1; k < s; k <<= 1)
        {
            ZeroOnePack(k * v, k * w);
            s -= k;
        }
        ZeroOnePack(s * v, s * w);
    }
}

void MixedPack(int v, int w, int s)
{
    if (s == -1)
        ZeroOnePack(v, w);
    else if (s == 0)
        CompletePack(v, w);
    else if (s > 0)
        MultiplePack(v, w, s);
}

int main()
{
    cin >> N >> V;

    int v, w, s;
    for (int i = 1; i <= N; i++)
    {
        cin >> v >> w >> s;
        MixedPack(v, w, s);
    }
    cout << dp[V];

    return 0;
}
```

### 5. 二维费用的背包问题

```c++
#include <bits/stdc++.h>
using namespace std;

const int MAXSIZE = 110;
int N, V, M, dp[MAXSIZE][MAXSIZE];

void ZeroOnePack(int v, int m, int w)
{
    for (int j = V; j >= v; j--)
        for (int k = M; k >= m; k--)
            dp[j][k] = max(dp[j][k], dp[j - v][k - m] + w);
}

int main()
{
    cin >> N >> V >> M;
    int v, m, w;
    for (int i = 1; i <= N; i++)
    {
        cin >> v >> m >> w;
        ZeroOnePack(v, m, w);
    }
    cout << dp[V][M];

    return 0;
}
```

### 6. 分组背包问题

```c++
#include <bits/stdc++.h>
using namespace std;

const int MAXSIZE = 110;
int N, V, v[MAXSIZE], w[MAXSIZE], dp[MAXSIZE];


int main()
{
    cin >> N >> V;
    for (int i = 1; i <= N; i++)
    {
        int S;
        cin >> S;
        for (int j = 1; j <= S; j++)
            cin >> v[j] >> w[j];

        for (int k = V; k >= 0; k--)
            for (int j = 1; j <= S; j++)
                if (k - v[j] >= 0)
                    dp[k] = max(dp[k], dp[k - v[j]] + w[j]);
    }
    cout << dp[V];

    return 0;
}
```

### 7. 有依赖的背包问题

```c++
#include <bits/stdc++.h>
using namespace std;

const int MAXSIZE = 110;
struct good {
    int v, w;
    vector<int> children;
} goods[MAXSIZE];
int N, V, dp[MAXSIZE][MAXSIZE];

void dfs(int root)
{
    // 因为有依赖关系, 每次调用 dfs 函数需要初始化 dp 的相应部分
    for (int i = goods[root].v; i <= V; i++)
        dp[root][i] = goods[root].w;

    for (int child : goods[root].children)
    {
        dfs(child);
        /*
         * 分组背包, 这里由于依赖关系有 j <- V ~ goods[root].v, 即已经
         * 选择了当前孩子的父结点, 至少占用了 goods[root].v, 故非为 V ~ 0.
        **/
        for (int j = V; j >= goods[root].v; j--)
            for (int k = 0; k <= j - goods[root].v; k++)
                dp[root][j] = max(dp[root][j], dp[root][j - k] + dp[child][k]);
    }
}

int main()
{
    cin >> N >> V;
    int root;
    for (int i = 1; i <= N; i++)
    {
        int father;
        cin >> goods[i].v >> goods[i].w >> father;
        if (father == -1)
            root = i;
        else
            goods[father].children.push_back(i);
    }
    dfs(root);
    cout << dp[root][V];

    return 0;
}
```

### 8. 背包问题最优选法方案数

```c++
#include <bits/stdc++.h>
using namespace std;

const int MAXSIZE = 1010;
const int MOD = 1e9 + 7;
int N, V, dp[MAXSIZE], cnt[MAXSIZE];

int main()
{
    cin >> N >> V;
    fill(dp + 1, dp + V + 1, INT_MIN);
    cnt[0] = 1;
    int v, w;
    for (int i = 1; i <= N; i++)
    {
        cin >> v >> w;
        for (int j = V; j >= v; j--)
        {
            int temp = max(dp[j], dp[j - v] + w), s = 0;
            if (temp == dp[j])
                s += cnt[j];
            if (temp == dp[j - v] + w)
                s += cnt[j - v];
            dp[j] = temp;
            cnt[j] = s % MOD;
        }
    }
    
    int maxW = 0, res = 0;
    for (int i = 0; i <= V; i++)
        maxW = max(maxW, dp[i]);
    for (int i = 0; i <= V; i++)
        if (dp[i] == maxW)
            res = (res + cnt[i]) % MOD;
    cout << res;

    return 0;
}
```